var gym = [
        {
        name : "Sylvester Stallone",
        quote : "Success is usually the culmination of controlling failure."
        },
        {
        name : "LeBron James",
        quote : "Don’t be afraid of failure. This is the way to succeed."
        },
        {
        name : "Michael Boyle",
        quote : "Most people give up right before the big break comes—don’t let that person be you."
        },
        {
        name : "Arnold Schwarzenegger",
        quote : "The last three or four reps is what makes the muscle grow. This area of pain divides the champion from someone else who is not a champion."
        }
];
var homework = [
        {
        name : "Herman Cain",
        quote : "Success is not the key to happiness. Happiness is the key to success. If you love what you are doing, you will be successful."
        },
        {
        name : "Pablo Picasso",
        qoute : "Action is the fundamental key to success."
        },
        {
        name : "Albert Einstein",
        quote : "Try not to become a man of success. Rather become a man of value."
        },
        {
        name : "Vince Lombardi",
        quote : "The difference between a successful person and others is not a lack of strength, not a lack of knowledge, but rather a lack in will."
        }
];
var work = [
        {
        name : "Phillip Sweet",
        quote : "Stay true to yourself, yet always be open to learn. Work hard, and never give up on your dreams, even when nobody else believes they can come true but you. These are not cliches but real tools you need no matter what you do in life to stay focused on your path."
        },
        {
        name : "Colin Powell",
        quote : "Succes is the result of perfection, hard work, learning from failure, loyalty, and persistence."
        },
        {
        name : "Newt Gingrich",
        quote : "Perseverance is the hard work you do after you get tired of doing the hard work you already did."
        },
        {
        name : "Conan O'Brien",
        quote : "Work hard, be kind, and amazing things will happen."
        }
];
var women = [
        {
        name : "Helen Keller",
        quote : "Optimism is the faith that leads to achievement."
        },
        {
        name : "Susan B. Anthony",
        quote : "Cautious, careful people, always casting about to preserve their reputations can never effect a reform."
        },
        {
        name : "Ann Richards",
        quote : "If you think taking care of yourself is selfish, change your mind. If you don’t, you’re simply ducking your responsibilities."
        },
        {
        name : "Oprah Winfrey",
        quote : "Step out of the history that is holding you back. Step into the new story you are willing to create."
        }
];
var mentalHealth = [
        {
        name : "Louise Hart",
        quote : "Self-esteem is as important to our well-being as legs are to a table. It is essential for physical and mental health and for happiness."
        },
        {
        name : "Matt Haig",
        quote : "We need, ultimately, to be able to view mental health with the same clear-headedness we show when talking about physical health."
        },
        {
        name : "Rosalynn Carter",
        quote : "If you look at suicides, most of them are connected to depression. And the mental health system just fails them. It's so sad. We know what to do. We just don't do it."
        },
        {
        name : "Diane Abbott",
        quote : "Mental health is often missing from public health debates even though it's critical to welbeing."
        }
];
var artisit = [
        {
        name : "Diplo",
        quote : "To me, rap music is bigger than who's the coolest rapper, the biggest rapper. It's everything about your personality."
        },
        {
        name : "Bob Ross",
        quote : "We don't make mistakes, just happy little accidents."
        },
        {
        name : "French Montana",
        quote : "I speak from the heart. Certain people follow lyricists and people that put words on a dictionary together, and this and that. I'm more of a rapper that speaks how I feel. I just tell it how it is."
        },
        {
        name : "Pablo Picasso",
        quote : "Every child is an artist. The problem is how to remain an artist once we grow up."
        }
];
var kids = [
        {
        name : "Giles Andreae",
        quote : "We all can dance when we find music we love."
        },   
        {
        name : "Maya Angelou",
        quote : "Do the best you can until you know better. Then when you know better, do better."
        },
        {
        name : "Natalie Portman",
        quote : "I don’t love studying. I hate studying. I like learning. Learning is beautiful."
        },
        {
        name : "Roald Dahl",
        quote : "If you have good thoughts they will shine out of your face like sunbeams and you will always look lovely."
        }   
];
var men = [
        {
        name : "A.A. Milne/Christopher Robin",
        quote : "You’re braver than you believe, and stronger than you seem, and smarter than you think."
        },
        {
        name : "Jim Rohn",
        quote : "Don’t just read the easy stuff. You may be entertained by it, but you will never grow from it."
        },
        {
        name : "Edwin Hubbel Chapin",
        quote : "Every action in our lives touches on some chord that will vibrate in eternity."
        },
        {
        name : "Theodore Roosevelt",
        quote : "In any moment of decision, the best thing you can do is the right thing. The worst thing you can do is nothing."
        }
];